...
async function convert(e){
  e.preventDefault();
  var reg = /^\d+$/;
  // !!! regex ucun test metodu ile yoxlaya bilersen
  a = selector1.value.slice(0, 3);
  // !!! selectoru slice etmek yerine apidan goturduyun datani ya slice etmelisen
// !!! ya da apidan fetch ederken limit qoyaraq etmek olar
...
const api2 = async (a, b)=>{
 ...
 const api1 = async ()=>{
  ...
  const changeJS = await url.json();
  // !!!
  const arr = changeJS.supported_codes.slice(0,3);
  // !!! meselen bu sekilde
